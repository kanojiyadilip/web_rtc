...
export class OperationReportComponent implements OnInit {
  ...
  public displayedColumns = ['No', 'DoctorName', 'ipdPatinetName', 'operationName', 'charges', 'operationDate'];
  ...
  overAllTotal: any;
	public onSubmit(value):void {
		...
		if (this.form.valid) {
			this.http.post(this._com.baseUrl+'/operationReport', {adminId: localStorage.getItem('adminId'), doctorId: value.doctorId, fromDate: value.fromDate, toDate: value.toDate}).subscribe(result=> {this.outPut = result;
  ...
  ExportTOExcel()
  {
    ...
    /* save to file */
    XLSX.writeFile(wb, 'Operation-report.xlsx');
