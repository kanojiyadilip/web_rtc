...
export class NurseAddComponent {
 ...
 image: any;
  ...
  public onSubmit(values):void {
    ...
    if (this.form.valid) {
      ...
      this.http.post(this._com.baseUrl+'/createNurse', {adminId: localStorage.getItem('adminId'), name: values.name, gender: values.gender, dob: values.dob, mobileNo1: values.mobileNo1, mobileNo2: values.mobileNo2, address: values.address, state: values.state, city: values.city, pinCode: values.pinCode,department: values.department, degree: values.degree, experience: values.experience, JoiningDate: values.JoiningDate, img: this.image}).subscribe(
        result => {
          this.outPut = result;
